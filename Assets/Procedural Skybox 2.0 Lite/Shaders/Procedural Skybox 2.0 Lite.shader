Shader "Skybox/Procedural Skybox 2.0 Lite"
{
    Properties
    {
        ...
        [Toggle] _EnableStars("Enable Stars", Float) = 0
        _StarSeed("Star Seed", Range(0,1000)) = 0
        _StarDensity("Star Density", Range(100,500)) = 200
        _StarIntensity("Star Intensity", Range(0.1,0.5)) = 0.25
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            static const float LITE_STAR_SIZE = 0.25;
            ...
            fixed4 ProcessSky(float3 dir)
            {
                ...
                float w = lerp(0.038, 0.002, saturate(_HorizonSharpness));
                float BLEND = max(w, 1e-4);
            ...
            fixed3 ProcessMoon(float3 dir)
            {
                if (_MoonSize <= 0) return 0;
                float3 sDir = normalize(_WorldSpaceLightPos0.xyz);
                float3 nDir = normalize(dir);
                float3 worldUp = abs(sDir.y) > 0.99 ? float3(1, 0, 0) : float3(0, 1, 0);
                float3 right = normalize(cross(worldUp, sDir));
                float3 up = cross(sDir, right);
                float2 p = float2(dot(nDir, right), dot(nDir, up));
                float r = length(p);
                float rad = max(_MoonSize, 1e-4);
                float2 uv = p / rad * 0.5 + 0.5;
                fixed3 texCol = tex2D(_MoonTex, saturate(uv)).rgb;
                float edge = rad * 0.1;
                float core = 1.0 - smoothstep(rad - edge, rad, r);
                fixed3 col = texCol * core;
                return saturate(col);
            }
            
            float Hash3(float3 p)
            {
                return frac(sin(dot(p, float3(12.9898, 78.233, 34.123))) * 43758.5453);
            }
            
            fixed3 ProcessStars(float3 dir)
            {
                if (_EnableStars < 0.5) return 0;
                float3 cell = dir * _StarDensity;
                float3 ip = floor(cell);
                float3 fp = frac(cell);
                float rnd = Hash3(ip + _StarSeed);
                float starCore = step(0.97, rnd);
                float dist = length(fp - 0.5);
                float s = DiscFade_HQ(dist, _StarIntensity);
                return starCore * s;
            }
